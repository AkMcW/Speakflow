...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { scenario, transcript, scores, fillerWords, wpm, durationSeconds, strengths, improvements, aiFeedback } = await req.json();
    const s = getSql();
    const rows = await s`
      INSERT INTO practice_sessions
        (user_id, scenario, transcript, scores, filler_words, wpm, duration_seconds, strengths, improvements, ai_feedback)
      VALUES
...
export async function GET() {
  ...
  try {
    ...
    const s = getSql();
    const rows = await s`
      SELECT id, scenario, scores, filler_words, wpm, duration_seconds, strengths, improvements, ai_feedback, created_at
